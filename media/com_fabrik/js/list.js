...
var FbList = new Class({
	...
	_csvExportForm: function () {
		...
		var c = new Element('form', {id: 'csvoptsform',
			'action': url,
			'method': 'post'
		});
		var excel = new Element('div',{'class': 'excel'}).inject(c);
		var excelHeader = new Element('h4', { text: Joomla.JText._('COM_FABRIK_FILE_TYPE') }).inject(excel);
		var excelLabel0 = new Element('label', { text: 'CSV'}).inject(excel);
		var checkval = this.options.csvOpts.excel===0;
		var excelInput0 = new Element('input', { 
			'type': 'radio',
			'name': 'excel',
			'value': '0'}).set('checked',this.options.csvOpts.excel==='0').inject(excelLabel0);
		var excelLabel1 = new Element('label', { text: 'Excel CSV'}).inject(excel);
		checkval = this.options.csvOpts.excel!==0;
		var excelInput1 = new Element('input', { 
			'type': 'radio',
			'name': 'excel',
			'value': '1'}).set('checked',checkval).inject(excelLabel1);
	
		var incfilters = new Element('div', {'class': 'incfilters'}).inject(c);
		var incfiltersHeader = new Element('h4', { text: Joomla.JText._('COM_FABRIK_INCLUDE_FILTERS')}).inject(incfilters);
		var incfiltersLabel1 = new Element('label', { text: Joomla.JText._('JYES')}).inject(incfilters);
		checkval = this.options.csvOpts.incfilters===1;
		var incfiltersInput1 = new Element('input', {
			'type': 'radio',
			'name': 'incfilters',
			'value': '1'}).set('checked',checkval).inject(incfiltersLabel1);
		var incfiltersLabel0 = new Element('label', { text: Joomla.JText._('JNO')}).inject(incfilters);
		checkval = this.options.csvOpts.incfilters===0;
		var incfiltersInput0 = new Element('input', {
			'type': 'radio',
			'name': 'incfilters',
			'value': '0'}).set('checked',checkval).inject(incfiltersLabel0);
			
		var inctabledata = new Element('div', {'class': 'inctabledata'}).inject(c);	
		var inctabledataHeader = new Element('h4', { text: Joomla.JText._('COM_FABRIK_INCLUDE_DATA')}).inject(inctabledata);
		var inctabledataLabel1 = new Element('label', { text: Joomla.JText._('JYES')}).inject(inctabledata);
		checkval = this.options.csvOpts.inctabledata===1;
		var inctabledataInput1 = new Element('input', {
			'type': 'radio',
			'name': 'inctabledata',
			'value': '1'}).set('checked',checkval).inject(inctabledataLabel1);
		var inctabledataLabel0 = new Element('label', { text: Joomla.JText._('JNO')}).inject(inctabledata);
		checkval = this.options.csvOpts.inctabledata===0;
		var inctabledataInput0 = new Element('input', {
			'type': 'radio',
			'name': 'inctabledata',
			'value': '0'}).set('checked',checkval).inject(inctabledataLabel0);
			
		var incraw = new Element('div', {'class': 'incraw'}).inject(c);	
		var incrawHeader = new Element('h4', { text: Joomla.JText._('COM_FABRIK_INCLUDE_RAW_DATA')}).inject(incraw);
		var incrawLabel1 = new Element('label', { text: Joomla.JText._('JYES')}).inject(incraw);
		checkval = this.options.csvOpts.incraw===1;
		var incrawInput1 = new Element('input', {
			'type': 'radio',
			'name': 'incraw',
			'value': '1'}).set('checked',checkval).inject(incrawLabel1);
		var incrawLabel0 = new Element('label', { text: Joomla.JText._('JNO')}).inject(incraw);
		checkval = this.options.csvOpts.incraw===0;
		var incrawInput0 = new Element('input', {
			'type': 'radio',
			'name': 'incraw',
			'value': '0'}).set('checked',checkval).inject(incrawLabel0);

		var inccalcs = new Element('div', {'class': 'inccalcs'}).inject(c);	
		var inccalcsHeader = new Element('h4', { text: Joomla.JText._('COM_FABRIK_INCLUDE_CALCULATIONS')}).inject(inccalcs);			
		var inccalcsLabel1 = new Element('label', { text: Joomla.JText._('JYES')}).inject(inccalcs);
		checkval = this.options.csvOpts.inccalcs===1;
		var inccalcsInput1 = new Element('input', {
			'type': 'radio',
			'name': 'inccalcs',
			'value': '1'}).set('checked',checkval).inject(inccalcsLabel1);
		var inccalcsLabel0 = new Element('label', { text: Joomla.JText._('JNO')}).inject(inccalcs);
		checkval = this.options.csvOpts.inccalcs===0;
		var inccalcsInput0 = new Element('input', {
			'type': 'radio',
			'name': 'inccalcs',
			'value': '0'}).set('checked',checkval).inject(inccalcsLabel0);

		var exportcols = new Element('div', {'class': 'exportcols'}).inject(c);		
		new Element('h4').set('text', Joomla.JText._('COM_FABRIK_SELECT_COLUMNS_TO_EXPORT')).inject(exportcols);
		var g = '';
		var i = 0;
		$H(this.options.labels).each(function (label, k) {
			if (k.substr(0, 7) !== 'fabrik_' && k !== '____form_heading') {
				...
				if (newg !== g) {
					g = newg;
					new Element('h4').set('text', g).inject(exportcols);
				}
				var rad = "<input type='radio' value='1' name='fields[" + k + "]' checked='checked' />" + Joomla.JText._('JYES');
				label = label.replace(/<\/?[^>]+(>|$)/g, "");
				var r = new Element('div').appendText(label);
				r.inject(exportcols);
				new Element('label').set('html', rad).inject(exportcols);
				new Element('label').adopt([new Element('input', {
					'type': 'radio',
					'name': 'fields[' + k + ']',
					'value': '0'
				}), new Element('span').appendText(Joomla.JText._('JNO'))]).inject(exportcols);
			}
			i++;
		}.bind(this));
		// elements not shown in table
		// Bauer changed - Why are these checked Yes by default???
		if (this.options.formels.length > 0) {
			var formfields = new Element('div', {'class': 'formfields'}).inject(c);				
			new Element('h4').set('text', Joomla.JText._('COM_FABRIK_FORM_FIELDS')).inject(formfields);
			this.options.formels.each(function (el) {
				var rad = "<input type='radio' value='1' name='fields[" + el.name + "]' />" +
					Joomla.JText._('JYES');
				var r = new Element('div').appendText(el.label);
				r.inject(formfields);
				new Element('label').set('html', rad).inject(formfields);
				new Element('label').adopt([new Element('input', {
					'type': 'radio',
					'name': 'fields[' + el.name + ']',
					'value': '0'
				}).set('checked',true), new Element('span').set('text', Joomla.JText._('JNO'))]).inject(formfields);
			}.bind(this));
		}
		var inputArray = c.getElementsByTagName('input');
		var number_inputs = inputArray.length ;
		for( var i = 0; i < number_inputs ; i++ ){
			var target = inputArray[ i ] ;
			target.setStyles({
				'float': 'left',
				'margin-right': '4px'});
		}	
		var labelArray = c.getElementsByTagName('label');
		var number_labels = labelArray.length ;
		for( var i = 0; i < number_labels ; i++ ){
			var target = labelArray[ i ] ;
			target.setStyles({
		...
		new Element('div', {
			...
			events: {
				'click': function (e) {
					...
					// Bauer changed to hide form elements and resize window after submit
					e.target.hide();
					c.hide();
					...
					$$('.contentWrapper').each(function(ele){
						ele.setStyle('height',null);
					});				
					document.id('exportcsv').setStyle('height',null);	
					csvMsg.set('html', Joomla.JText._('COM_FABRIK_LOADING') + ' <br /><span id="csvcount">0</span> / <span id="csvtotal"></span> ' + Joomla.JText._('COM_FABRIK_RECORDS') + '.<br/>' + Joomla.JText._('COM_FABRIK_SAVING_TO') + ': <span id="csvfile"></span>').setStyle('text-align', 'center');
			...
			onComplete: function (res) {
				if (res.err) {
					...
					if (res.count < res.total) {
						...
						// Bauer added -  close the window else any subsequent click on download link will cause error because file has been deleted						
						jQuery('#exportcsv div.alert-success p a.btn-success').focusout(function(){
							delete(Fabrik.Windows.exportcsv);
							jQuery('#exportcsv').remove();
						});	
					}
				}
			}.bind(this)
